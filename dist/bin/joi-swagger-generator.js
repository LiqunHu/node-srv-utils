...
let argv = yargs_1.default.alias('v', 'validator')
    .alias('o', 'output')
    .alias('h', 'header')
    .alias('b', 'baseUrl')
    .alias('m', 'mapPath')
    ...
    .demandOption(['v', 'o', 'h'])
    .help('help')
    .example('joi-swagger-generator', 'joi-swagger-generator -r -v ./validators -h ./header.json -o ./swagger.json').argv;
...
const validatorFile = path_1.default.resolve(relativeValidatorPath);
    ...
    for (let key in apiList) {
        ...
        if (currentValue.JoiSchema) {
            if (currentValue.JoiSchema.header) {
                const { swagger } = (0, joi_to_swagger_1.default)(currentValue.JoiSchema.header);
            ...
            if (currentValue.JoiSchema.body) {
                ...
                const { swagger } = (0, joi_to_swagger_1.default)(currentValue.JoiSchema.body);
            ...
            if (currentValue.JoiSchema.path) {
                const { swagger } = (0, joi_to_swagger_1.default)(currentValue.JoiSchema.path);
            ...
            if (currentValue.JoiSchema.query) {
                const { swagger } = (0, joi_to_swagger_1.default)(currentValue.JoiSchema.query);
            ...
            if (currentValue.JoiSchema.response) {
                let responses = {};
                const { swagger } = (0, joi_to_swagger_1.default)(currentValue.JoiSchema.response);
...
if (argv.r) {
    (0, glob_1.default)(path_1.default.join(validatorFile, '**/*.validator.ts'), function (er, files) {
        console.log(files);
        let requires = [];
        ...
        const headerFile = path_1.default.resolve(relativeHeaderPath);
        if (!fs_extra_1.default.pathExistsSync(headerFile)) {
            return console.error(`Header file not found in ${headerFile}, please create header file first`);
        }
        else {
            try {
                fs_extra_1.default.ensureFileSync(headerFile);
            }
            catch (e) {
                return console.error(`Header file not found in ${headerFile}, please create header file first`);
            }
        ...
        const outputFile = path_1.default.resolve(relativeOutputFile);
        for (let r of requires) {
            json = applyLogic(json, r.apiList);
        }
        fs_extra_1.default.outputFile(outputFile, JSON.stringify(json, null, 4), function (err) {
            if (err) {
                console.error(err);
                process.exit(1);
            }
    ...
    if (!fs_extra_1.default.pathExistsSync(validatorFile)) {
        console.error(`Validator file not found in ${validatorFile}, please create validator file first`);
        process.exit(1);
    }
    else {
        try {
            fs_extra_1.default.ensureFileSync(validatorFile);
        }
        catch (e) {
            console.error(`Validator file not found in ${validatorFile}, please create validator file first`);
            process.exit(1);
...
function getApiGatewayIntegration(currentValue, convertedPath, mapHeader, requestMap) {
    ...
    let requestPath = Object.assign({}, requestMap);
    const splitPath = convertedPath.split('/');
    for (const i in splitPath) {
        let eachPath = splitPath[i];
